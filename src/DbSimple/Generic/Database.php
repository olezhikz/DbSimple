...
use DbSimple\Generic\LastError as DbSimpleGenericLastError;
/**
 ...
 */
class Database extends DbSimpleGenericLastError
{
    ...
    */
    ...
    function _expandPlaceholdersCallback($m)
    {
        ...
        if(isset($m[1]) && strlen($block=$m[1])){
            ...
            $block = $this->_placeholderNoValueFound ? '' : ' '.$this->_expandPlaceholdersFlow($block).' ';
    ...
    function _setLastError($code, $msg, $query)
    {
        ...
        return DbSimpleGenericLastError::_setLastError($code, $msg, $query);
    }
    ...
    function _fieldList2Count($fields)
    {
        ...
        if(preg_match('/^\s* DISTINCT \s* (.*)/sx', $fields, $m)){
            $fields = preg_replace('/\s+ AS \s+ .*? (?=,|$)/sx', '', $m[1]);
