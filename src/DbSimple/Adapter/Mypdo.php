...
class Mypdo extends Database implements AdapterInterface, DatabaseInterface {
    ...
    public function __construct($dsn) {
        ...
        try {
            ...
            if (!empty($dsn['socket'])) {
                // Socket connection
                $dsnPdo = 'mysql:unix_socket=' . $dsn['socket'] . ';dbname=' . $base;
                if ($phpNew) {
                    $dsnPdo .= ';charset=' . (!empty($dsn['enc']) ? $dsn['enc'] : 'utf8');
                }
                $this->link = new PDO($dsnPdo, isset($dsn['user']) ? $dsn['user'] : 'root', isset($dsn['pass']) ? $dsn['pass'] : '', $options);
            } else if (!empty($dsn['host'])) {
                // Host connection

                $dsnPdo = 'mysql:host=' . $dsn['host'] . (empty($dsn['port']) ? '' : ';port=' . $dsn['port']) . ';dbname=' . $base;
                if ($phpNew) {
                    $dsnPdo .= ';charset=' . (!empty($dsn['enc']) ? $dsn['enc'] : 'utf8');
                }

                $this->link = new PDO($dsnPdo, $dsn['user'], isset($dsn['pass']) ? $dsn['pass'] : '', $options);
            } else {
