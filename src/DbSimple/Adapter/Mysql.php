...
class Mysql extends Database implements AdapterInterface, DatabaseInterface {
    ...
    function __construct($dsn) {
        ...
        $ok = $this->link = call_user_func($connect, $str = $dsn['host'] . (empty($dsn['port']) ? "" : ":" . $dsn['port']), $dsn['user'] = empty($dsn['user']) ? '' : $dsn['user'], $dsn['pass'] = empty($dsn['pass']) ? '' : $dsn['pass'], true
        );
        $this->_resetLastError();
        if (!$ok) {
            if (!$ok) {
                return $this->_setDbError('mysql_connect("' . $str . '", "' . $dsn['user'] . '")');
            }
        }
        $ok2 = mysql_select_db(preg_replace('{^/}s', '', $dsn['path']), $this->link);
        if (!$ok2) {
